...
//********成員基本資訊和參與了什麼合作案的介面***********
class JoinVerify extends Component {
  ...
  async getCooJson(ipfshash,i,coolen){
    ...
    await request(`https://ipfs.io/ipfs/${ipfshash}`, async function (error, response, body) {
      if (!error && response.statusCode === 200) {
        let importedJSON = JSON.parse(body);
        if(importedJSON['host']===this.state.account&&!await platform.methods.cooWaitingVerified(importedJSON['ID']).call()){
            this.setState({
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th scope="col">wating verified</th>
            </tr>
          </thead>
          <tbody id="request">
