...
class RequestList extends Component {
  ...
  async getInit(){
    //從合作案的JSON中獲得資料集的數量
    let datasetLen = this.props.location.state.cooperationJson['unVerifiedMenData'].length
    if(datasetLen===0) return this.setState({isLoading:false})
    //從資料集中找出該成員提供的欄位
    for (let i = 0; i < datasetLen; i++){
      //資料集對應成員地址相符
      if(this.props.location.state.cooperationJson['unVerifiedMenData'][i][0]===this.props.location.state.memAddress){
        const datasetHash =  await platform.methods.getDataset(this.props.location.state.cooperationJson['unVerifiedMenData'][i][1]).call()
